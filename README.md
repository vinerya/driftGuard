...
```bash
pip install driftguard[viz]        # matplotlib + scikit-learn for corpus.plot()
pip install driftguard langgraph   # LangGraph guardrail nodes
```
...
Clustering uses numpy k-means internally — no extra dependencies.
...
```python
# You have 500 example legal responses — pick the 30 most diverse ones.
